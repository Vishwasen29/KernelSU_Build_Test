...
HOOK_SCRIPT="${3:-$(dirname "$0")/kernel-4.19-kernelsu-next-legacy-hooks.sh}"
...
# legacy_susfs is an explicit branch request from the user
...
mkdir -p "$DRIVER_DIR"
ln -snf ../../KernelSU-Next/kernel "$DRIVER_DIR/kernelsu" 2>/dev/null || ln -snf ../KernelSU-Next/kernel "$DRIVER_DIR/kernelsu"

grep -q 'kernelsu' "$DRIVER_DIR/Makefile" || printf '\nobj-$(CONFIG_KSU) += kernelsu/\n' >> "$DRIVER_DIR/Makefile"
grep -q 'drivers/kernelsu/Kconfig' "$DRIVER_DIR/Kconfig" || printf '\nsource "drivers/kernelsu/Kconfig"\n' >> "$DRIVER_DIR/Kconfig"
