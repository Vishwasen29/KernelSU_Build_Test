...
insert_once_after_any() {
  local file="$1" block="$2"
  shift 2
  local needle
  for needle in "$@"; do
    if grep -Fq "$needle" "$file"; then
      insert_once_after "$file" "$needle" "$block"
      return 0
    fi
  done
...
\t\t\t\t      int *__never_use_flags);
    ...
    insert_once_after "$f" "SYSCALL_DEFINE3(execve, const char __user *, filename," '#ifdef CONFIG_KSU
\tksu_handle_execve_sucompat(&filename, NULL, NULL, NULL);
#endif'

    if grep -q "COMPAT_SYSCALL_DEFINE3(execve," "$f"; then
      insert_once_after_any "$f" '#ifdef CONFIG_KSU
\t{
\t\tksu_handle_execve_sucompat(&filename, NULL, NULL, NULL);
\t}
#endif' \
        "COMPAT_SYSCALL_DEFINE3(execve, const char __user *, filename," \
        "COMPAT_SYSCALL_DEFINE3(execve, compat_uptr_t, filename,"
    fi
...
\t\t\t\t      void *envp, int *flags);
    ...
    insert_once_after "$f" "int do_execve(struct filename *filename," '#ifdef CONFIG_KSU
\tksu_handle_execveat((int *)AT_FDCWD, &filename, &argv, &envp, 0);
#endif'

    if grep -q "static int compat_do_execve(struct filename \*filename," "$f"; then
      insert_once_after "$f" "static int compat_do_execve(struct filename *filename," '#ifdef CONFIG_KSU
\tksu_handle_execveat((int *)AT_FDCWD, &filename, &argv, &envp, 0);
#endif'
    fi
...
\t\t\t\t      int *flags);
  ...
  insert_once_after "$f" "SYSCALL_DEFINE3(faccessat, int, dfd, const char __user *, filename, int, mode)" '#ifdef CONFIG_KSU
...
\t\t\t\t       size_t *count_ptr);
    ...
    insert_once_after "$f" "SYSCALL_DEFINE3(read, unsigned int, fd, char __user *, buf, size_t, count)" '#ifdef CONFIG_KSU
...
\t\t\t\t      size_t *count_ptr, loff_t **pos);
    ...
    insert_once_after "$f" "ssize_t vfs_read(struct file *file, char __user *buf, size_t count, loff_t *pos)" '#ifdef CONFIG_KSU
\tif (unlikely(ksu_vfs_read_hook))
...
extern int ksu_handle_stat(int *dfd, const char __user **filename_user, int *flags);
  ...
  if grep -q "SYSCALL_DEFINE4(newfstatat, int, dfd," "$f"; then
    insert_once_after "$f" "SYSCALL_DEFINE4(newfstatat, int, dfd," '#ifdef CONFIG_KSU
\tksu_handle_stat(&dfd, &filename, &flag);
#endif'
  fi
  if grep -q "SYSCALL_DEFINE4(fstatat64, int, dfd," "$f"; then
    insert_once_after "$f" "SYSCALL_DEFINE4(fstatat64, int, dfd," '#ifdef CONFIG_KSU
\tksu_handle_stat(&dfd, &filename, &flag);
#endif'
  fi
...
patch_reboot() {
  ...
  insert_once_after "$f" "SYSCALL_DEFINE4(reboot, int, magic1, int, magic2, unsigned int, cmd," '#ifdef CONFIG_KSU
...
log "[+] Done. Review changes with: git diff -- fs/exec.c fs/open.c fs/read_write.c fs/stat.c kernel/reboot.c"
