...
/* SUSFS exported SID variables - used by fs/susfs.c, security/selinux/avc.c,
 * and drivers/kernelsu/setuid_hook.c via extern declarations.
 * Populated in cache_sid() once SELinux policy is loaded. */
...
void cache_sid(void)
{
    ...
    /* Populate SUSFS exported SID aliases */
    susfs_ksu_sid = cached_su_sid;
    susfs_zygote_sid = cached_zygote_sid;

    /* priv_app SID - used for AVC log spoofing */
    err = security_secctx_to_secid("u:r:priv_app:s0", strlen("u:r:priv_app:s0"),
                                   &susfs_priv_app_sid);
    if (err) {
...
/*
 * SUSFS compatibility shims
 * These bridge the gap between SUSFS's expected symbols and KernelSU's
 * internal naming conventions.
 */

/* Called by fs/susfs.c - wraps KSU's is_ksu_domain() */
bool susfs_is_current_ksu_domain(void)
{
    return is_ksu_domain();
}

/* Called by drivers/kernelsu/setuid_hook.c
 * Compares cred->security SID to a target u32 SID.
 * cred->security points to a task_security_struct whose first field is sid. */
bool susfs_is_sid_equal(void *security, u32 sid)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 18, 0)
    const struct task_security_struct *tsec =
        (const struct task_security_struct *)security;
#else
